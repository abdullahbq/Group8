...
exports.createPages = ({ graphql, actions }) => {
  ...
  const movieItemTemplate = path.resolve(
    `./src/templates/movie-item-template.js`
  )
  return graphql(
    `
      {
    ...
    const shopItems = allPosts.filter(
      edge => edge.node.fields.collection === `movies`
    )
    ...
    shopItems.forEach((post, index, arr) => {
      ...
      createPage({
        path: `/movie${post.node.fields.slug}`,
        component: movieItemTemplate,
        context: {
          slug: post.node.fields.slug,
          prev: prev,
          next: next,
